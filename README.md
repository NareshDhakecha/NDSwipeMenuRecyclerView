...
```groovy
dependencies {
    implementation 'com.github.NareshDhakecha:NDSwipeMenuRecyclerView:1.0.0'
}
```
...
```groovy
-keep class * implements com.ndsoftwares.swipe_rv.NDSwipeLayout$Designer
```
...
```xml
<com.ndsoftwares.swipe_rv.NDSwipeLayout
    app:autoClose="true"
...
If use it as RecyclerView’s item, need to use `NDSwipeMenuRecyclerView` instead of  `RecyclerView`.

> `NDSwipeMenuRecyclerView` is almost the same as `RecyclerView`. It only slightly interferes with `dispatchTouchEvent()` to allow `NDSwipeLayout` automatically close menu when user scrolled the `RecyclerView`, and prevent open multiple menus during multi-touch.
>
> But if user manually calls the `RecyclerView.scroollxxx()` method to scoll the list, `NDSwipeMenuRecyclerView` will not automatically close  menu, because it doesn’t know whether the user needs this behavior.
>
> In this case, you can use the `NDSwipeMenuRecyclerView.closeMenus()` method.
...
```kotlin
swipeLayout.addListener(object : NDSwipeLayout.Listener {
...
You can implement `NDSwipeLayout.Designer` to customize more styles.
...
class ClassicDesigner : Designer {
    ...
    override fun onInit(parent: NDSwipeLayout, leftMenu: View?, rightMenu: View?) {
...
- `onInit()` is called after `onMeasure` of `NDSwipeLayout` method，and it will only be called once.

- `onLayout()` is called after `onLayout` of `NDSwipeLayout` method，and is called back every time when user sliding.
